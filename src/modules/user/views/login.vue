...
<script lang="ts">
import ILoginRequest from "../interface/login/login-request.interface";
...
import PurchaseOrderRouteName from "@/modules/purchase-order/enums/route-name.enum";
  ...
  setup() {
    ...
    const onLogin = async () => {
      userService.login(data).then((isSuccess: boolean) => {
        if (isSuccess) {
          router.push({
            name: PurchaseOrderRouteName.HOME,
          });
        } else {
          router.push({
